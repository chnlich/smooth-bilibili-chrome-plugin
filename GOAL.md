...
- 扩展接管视频页媒体分片的下载，代播放器发起同一 URL 的同一 Range 请求并可提前预取；分片只在下载层实例内存中保存，离开视频路由或页面关闭即释放，不落盘；但不接管播放：不自建播放管线、不替换媒体地址、不改变清晰度、倍速、seek、播放暂停、音视频轨或音量的任何决策。Bilibili 和用户继续拥有播放与媒体所有权。弹窗只显示当前播放位置覆盖的原生连续缓存秒数；没有覆盖区间就是 0 秒。
