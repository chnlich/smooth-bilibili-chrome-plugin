...
- `/video/*` 与 `/list/watchlater*` 使用同一视频增强。扩展对视频页媒体分片接管下载，代播放器发起同一 URL 的同一 Range 请求并可提前预取；仍只对当前原生播放器尝试一次 120 秒稳定缓存目标，并只读显示覆盖当前播放点的 `video.buffered` 连续区间。不接管播放，不调用 `play()`/`pause()`，不写播放位置、倍速、画质、音量、静音、source，也不改变播放器的清晰度、seek、播放暂停或音视频轨决策。
