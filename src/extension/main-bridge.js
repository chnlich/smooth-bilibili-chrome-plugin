...
function getCoreSnapshot() {
  ...
  const snapshot = {
    ...
    supportedQualityList:
      typeof core.getSupportedQualityList === 'function'
        ? serializeObserved(core.getSupportedQualityList())
        : undefined,
    qualityList:
      typeof core.getQualityList === 'function' ? serializeObserved(core.getQualityList('video')) : undefined,
...
async function callCoreAsync(args) {
  ...
  return {
    result: result === undefined ? true : serializeObserved(result),
