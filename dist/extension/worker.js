(() => {
  ...
  function selectEvictions({
    ...
    const perVideo = /* @__PURE__ */ new Map();
    ...
    while (currentTotal > maxBankBytes || [...currentPerVideo.values()].some((value) => value > maxBankBytesPerVideo)) {
      const overVideo = new Set([...currentPerVideo.entries()].filter(([, value]) => value > maxBankBytesPerVideo).map(([key]) => key));
  ...
  // src/bank/storage.js
  var BANK_DATABASE_NAME = "bilibili-media-segment-bank";
  var BANK_DATABASE_VERSION = 1;
  var BANK_CHUNK_STORE = "chunks";
  ...
  function openBankDatabase(indexedDbObject = globalThis.indexedDB) {
    if (indexedDbObject === void 0 || typeof indexedDbObject.open !== "function") {
  ...
  function mergeSegments(existing, next) {
    ...
    for (const segment of all) {
      ...
      if (previous === void 0 || segment.start > previous.end + 1) {
        merged.push(segment);
        continue;
      }
  ...
  async function readRangeRecords(database, bankKeyValue, start, end) {
    ...
    return records.filter((record) => record !== void 0);
  }
  async function readBankRange({
    ...
    try {
      ...
      if (bytes === void 0) return { hit: false, totalSize };
      return { hit: true, bytes, totalSize };
    } finally {
      database.close();
    }
  }
  async function writeBankChunk({
    bankKey: bankKeyValue,
    videoKey,
    start,
    end,
  ...
  async function enforceBankLimits({
    ...
    try {
      ...
      if (selected.entries.length === 0) return { evictedBytes: 0, reason: void 0, records: [] };
      await new Promise((resolve, reject) => {
        const transaction = database.transaction(BANK_CHUNK_STORE, "readwrite");
        const store = transaction.objectStore(BANK_CHUNK_STORE);
      ...
      const initialPerVideo = /* @__PURE__ */ new Map();
      for (const record of candidates) {
        const group = record.videoKey || record.bankKey;
        initialPerVideo.set(group, (initialPerVideo.get(group) || 0) + record.byteLength);
      }
      const reason = [...initialPerVideo.values()].some((value) => value > maxBankBytesPerVideo) ? "video_limit" : initialTotal > maxBankBytes ? "global_limit" : "limit";
  ...
  async function handleBankMessage(message, indexedDbObject) {
    if (!isBankMessage(message) || message.direction !== "request" || !Number.isInteger(message.requestId)) return void 0;
  ...
  function installBankWorker(runtimeObject = globalThis.chrome?.runtime) {
    ...
    runtimeObject.onMessage.addListener((message, _sender, sendResponse) => {
      ...
      void handleBankMessage(message).then((value) => sendResponse({
